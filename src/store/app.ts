...
- Пути с пробелами РАБОТАЮТ. В shell-командах оборачивай их в кавычки: cd "path with spaces" && npm run build. НИКОГДА не отказывай пользователю из-за пробелов в пути`);
...
## Выполнение
- **run_command(command, timeout_secs?)** — shell-команда в корне проекта. Используй для: сборки, тестов, установки зависимостей, git, линтера, проверки результата. Таймаут по умолчанию 120с, макс 300с. Команда выполняется через sh -c (Unix) / cmd /C (Windows) — пути с пробелами работают, просто оборачивай в кавычки.`);
